...
unsigned int image_find_raise_fall_edges_by_dir(
        const struct image *img, const struct point pstart,
        const struct point dir, struct image_raise_fall_edge *pedge, const unsigned int num)
{
    unsigned int cnt;
    int imgdata_off;
    int r, i, j, off_x, off_y;
    unsigned char grad;
    unsigned char gray;
    unsigned char cur_type;
    unsigned char last_type;
    ...
    cur_edge = pedge - 1;
    last_type = IMAGE_RFEDGE_TYPE_NONE;
    for (j = 0, r = 1; j < 2; ++j, r *= -1) {
        imgdata = img->data + pstart.y * img->width + pstart.x;
        gray = *imgdata;
        off_x = r * dir.x;
        off_y = r * dir.y;
        off_end.x = pstart.x + off_x;
        off_end.y = pstart.y + off_y;
        imgdata_off = off_y * img->width + off_x;
        imgdata += imgdata_off;
        for (i = r; off_end.x >= 0 && off_end.x < img->width && off_end.y >= 0 && off_end.y < img->height;
            i += r, imgdata += imgdata_off, off_end.x += off_x, off_end.y += off_y) {
            if (gray == *imgdata) {
                last_type = IMAGE_RFEDGE_TYPE_NONE;
                continue;
            } else if (gray < *imgdata) {
