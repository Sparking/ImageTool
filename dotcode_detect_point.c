...
#if 0
int get_edgepos_16x(const int *grads, const int len, const int srchpos, int *begin, int *end)
{
    int pos16x, i, j;
...
static inline bool int_same_sign(const int a, const int b)
{
    const int sign_bit = ((int)1) << ((sizeof(int) << 3) - 1);

    return ((a & sign_bit) == (b & sign_bit));
}

int get_grad_bepos(int *grads, const int len, const int srchpos, const bool isforward)
{
    int i;
    int ref;

    ref = abs(grads[srchpos] * 2 / 5);
    if (isforward) {
        for (i = srchpos - 1; i >= 0; --i) {
            if (!int_same_sign(grads[srchpos], grads[i]) || abs(grads[i]) < ref)
                break;
        }
        ++i;
    } else {
        for (i = srchpos + 1; i < len; ++i) {
            if (!int_same_sign(grads[srchpos], grads[i]) || abs(grads[i]) < ref)
                break;
        }
        --i;
...
bool get_dots_edge(unsigned char *data, const int len, const int center, int *center_offset, int *w16x, bool isblack)
{
    ...
    memset(gradabs, 0, sizeof(gradabs));
    ...
    if (j < 1) {
        ...
        i = index[j - 1];
        j = index[j];
        if (int_same_sign(grads[i], grads[j]) || grads[i] == 0 || grads[j] == 0) {
    ...
    if (int_same_sign(grads[i], grads[j]) || grads[i] == 0 || grads[j] == 0
        || ((isblack && grads[i] < 0) || (!isblack && grads[i] > 0))) {
        return false;
    }

    tmp = 0;
    npos = 0;
    b = i = get_grad_bepos(grads, len - 1, i, true);
    j = get_grad_bepos(grads, len - 1, j, false);
    if (isblack) {
        while (i <= j) {
            tmp += data[i];
            npos += i * data[i];
    ...
    *w16x = (j - b) << 4;
    *center_offset = (npos << 4) / tmp - (center << 4);
...
struct point dbgrefoff = { 1, 1 };
struct point dbgrefdot = { 226, 250 };
static inline bool dotcoincide(const struct point *pt)
{
    return (pt->x + dbgrefoff.x >= dbgrefdot.x) && (pt->x - dbgrefoff.x <= dbgrefdot.x) &&
        (pt->y + dbgrefoff.y >= dbgrefdot.y) && (pt->y - dbgrefoff.y <= dbgrefdot.y);
}
